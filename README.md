...
- ✅ List all students
- ✅ View registered modules per student

### Module Management:
- ✅ Add new modules
- ✅ Edit existing modules
- ✅ View individual module details
- ✅ List all modules
...
- ✅ Module details (code, name, description, credits)
...
- `VIEW_MODULES` - Get all available modules
- `REGISTER_MODULE` - Register for a module
- `VIEW_REGISTERED_MODULES` - Get registered modules

### Admin Operations - Students:
- `ADD_STUDENT` - Add new student
- `EDIT_STUDENT` - Update student info
- `VIEW_STUDENT` - Get student details
- `LIST_STUDENTS` - Get all students

### Admin Operations - Modules:
- `ADD_MODULE` - Add new module
- `EDIT_MODULE` - Update module info
- `VIEW_MODULE` - Get module details
...
Browser (HTML/CSS/JS)
    ↕ WebSocket
WebSocket Gateway (Port 8080)
    ↕ TCP Socket + Object Serialization
TCP Server (Port 8888)
    ↕ Multithreading
Data Storage (In-Memory ConcurrentHashMap)
```

## 🛠️ Networking Concepts Used

1. **TCP Sockets**: Reliable connection-oriented communication
2. **Multithreading**: Concurrent client handling using thread pools
3. **Object Serialization**: Converting objects to byte streams
4. **WebSocket Protocol**: Full-duplex communication for web browsers
5. **Client-Server Architecture**: Centralized server handling multiple clients
6. **Request-Response Pattern**: Structured communication protocol

## 📝 Notes

- No external libraries required (pure Java implementation)
- No database files - all data stored in-memory
- Server must be running before opening frontend
- Data is lost when server stops (in-memory storage)
- Thread-safe using `ConcurrentHashMap` for data storage
...
- Browser-to-backend integration without REST APIs
